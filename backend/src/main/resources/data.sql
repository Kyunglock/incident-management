...
-- 반영 이력
INSERT INTO release_history (id, release_plan_id, deployed_at, status, memo, created_at)
SELECT 1, 1, NOW() - INTERVAL '6 day', 'DEPLOYED', '운영 정상 반영 완료', NOW() - INTERVAL '6 day'
WHERE NOT EXISTS (SELECT 1 FROM release_history WHERE id = 1);

INSERT INTO release_history (id, release_plan_id, deployed_at, status, memo, created_at)
SELECT 2, 1, NOW() - INTERVAL '5 day', 'ROLLED_BACK', '결제 오류로 롤백', NOW() - INTERVAL '5 day'
WHERE NOT EXISTS (SELECT 1 FROM release_history WHERE id = 2);

INSERT INTO release_history (id, release_plan_id, deployed_at, status, memo, created_at)
SELECT 3, 2, NOW() - INTERVAL '4 day', 'DEPLOYED', '핫픽스 반영', NOW() - INTERVAL '4 day'
WHERE NOT EXISTS (SELECT 1 FROM release_history WHERE id = 3);

INSERT INTO release_history (id, release_plan_id, deployed_at, status, memo, created_at)
SELECT 4, 3, NOW() - INTERVAL '1 day', 'PENDING', '배포 대기 중', NOW() - INTERVAL '1 day'
